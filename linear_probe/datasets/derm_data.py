...
class Derm_Dataset(Dataset):
    def __init__(self, df: pd.DataFrame, root, image_key='filename', train=False,val=False,test=False,transforms=None, data_percent=1, label_key='label'):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.transforms:
            x = self.transforms(x)
            y = self.df.iloc[index][self.label_key]
        return x,y,im_path
