    ...
    for batch_idx, (batch, target, filename) in tqdm(
        ...
        with torch.inference_mode():
            if args.model.startswith('open_clip_'):
                # O-MAKE's vision tower returns (pooled_features, patch_embeddings).
                # Probe the un-normalised pooled features, which is what
                # CLIP.encode_image(normalize=False) yields in vanilla open_clip.
                out = model.visual(batch)
                features = out[0] if isinstance(out, (tuple, list)) else out
                embeddings = features.detach().cpu()[:remaining, :]
            else:
                # timm-style encoders (vit-base-16, dinov2, ...) return a tensor.
