...
# Runnable from any working directory: this package plus the repo's src/ (which
# provides the O-MAKE open_clip) must be importable.
...
from models.open_clip_model import OpenCLIPClassifier
...
def main(args, ds_init):
    ...
    if args.weights:
        ...
        if args.nb_classes == 2:
            train_y = df[(df['split'] == 'train')]['hier-target'].values.tolist()
        else:
    ...
    if args.model=="PanDerm_Large_FT":
    ...
    elif args.model == 'open_clip_vit_base_16':
        ...
        # 如果加载 checkpoint（仅加载 backbone 权重）
        ...
        patch_size = 16
    elif 'hf-hub:' in args.model:

        model, _ = create_model_from_pretrained(
            args.model.replace('open_clip_', '')
        )
    ...
    elif args.model == 'open_clip_ViT-B-16-SigLIP':
